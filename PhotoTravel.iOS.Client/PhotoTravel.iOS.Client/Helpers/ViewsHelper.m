...
+ (UIImage *)circularScaleNCrop:(UIImage *)sourceImage
  ...
  UIGraphicsBeginImageContextWithOptions(
      CGSizeMake(rect.size.width, rect.size.height), NO, 0.0);
...
+ (void)changeImageSourceWithAnimation:(UIImage *)imageToLoad
  ...
  void (^newImageLoading)() = ^void() {
      imageContainer.image = imageToLoad;
      [UIView animateWithDuration:0.5
                       animations:^{ imageContainer.alpha = 1; }];
  };

  [UIView animateWithDuration:0.2
      animations:^(void) { imageContainer.alpha = 0; }
